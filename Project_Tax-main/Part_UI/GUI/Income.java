 ...
 * ผู้พัฒนา: Pitchanan / ปรับปรุงโดยภูริ
 */
public class Income extends javax.swing.JFrame {
    ...
    public Income(String userId) {
        ...
        // ====== จำกัดเฉพาะตัวเลข และจำนวนหลัก ======
        inputyear.addKeyListener(new java.awt.event.KeyAdapter() {
            public void keyTyped(java.awt.event.KeyEvent evt) {
                char c = evt.getKeyChar();
                if (!Character.isDigit(c) || inputyear.getText().length() >= 4) {
                    evt.consume(); // ตัดไม่ให้พิมพ์เกิน 4 ตัว หรือไม่ใช่ตัวเลข
                }
            }
        });
    ...
    public Income(String userId2, String year, double income, double bonus, double other) {
    ...
    private void initComponents() {
        ...
        NextButton.addActionListener(e -> {
            ...
            if (year.length() != 4) {
                JOptionPane.showMessageDialog(
                    this,
                    "กรุณากรอกปีให้ถูกต้อง (4 หลัก เช่น 2025)",
                    "ปีไม่ถูกต้อง",
                    JOptionPane.WARNING_MESSAGE
                );
                return;
            ...
            if (!isYearExistInTaxData(year)) {
                JOptionPane.showMessageDialog(
                    this,
                    "ไม่มีข้อมูลปี " + year + " ในฐานข้อมูลภาษี\nโปรดตรวจสอบอีกครั้ง",
                    "ไม่พบข้อมูลปีนี้",
                    JOptionPane.WARNING_MESSAGE
                );
                return;
            ...
            // ✅ แปลงค่าที่กรอกเป็นตัวเลข และส่งไป TaxDeduction
            double salaryVal = Double.parseDouble(salary);
            double bonusVal = Double.parseDouble(bonus);
            double otherVal = Double.parseDouble(another);
        
            dispose();
            new TaxDeduction(userId, year, salaryVal, bonusVal, otherVal).setVisible(true);
        });
    ...
    // ======================== ตรวจปีในฐานข้อมูล ========================
    private boolean isYearExistInTaxData(String year) {
        File taxFile = new File("Part_Data/tax_data.csv");
        if (!taxFile.exists()) {
            JOptionPane.showMessageDialog(
                this,
                "ไม่พบไฟล์ฐานข้อมูลภาษี (tax_data.csv)",
                "ข้อผิดพลาด",
        ...
        try (BufferedReader br = new BufferedReader(new FileReader(taxFile))) {
            ...
            while ((line = br.readLine()) != null) {
                String[] p = line.split(",");
                if (p.length > 0 && p[0].trim().equals(year)) {
                    return true;
                }
            }
        } catch (IOException e) {
            ...
            JOptionPane.showMessageDialog(
                this,
                "เกิดข้อผิดพลาดในการอ่านไฟล์ฐานข้อมูลภาษี",
                "ข้อผิดพลาด",
                JOptionPane.ERROR_MESSAGE
            );
        }
    ...
    private void saveIncomeData(String year, String salary, String bonus, String another) {
        String fileName = "Part_Data/user_data.csv";
        ...
        if (!found) {
            ...
            String newLine = String.join(",",
                userId, year,salary, bonus, "โสด", "-", "-", "-", "-", "0",
                "0", "0", "0", "0", "0", "0"
            );
            lines.add(newLine);
        }
