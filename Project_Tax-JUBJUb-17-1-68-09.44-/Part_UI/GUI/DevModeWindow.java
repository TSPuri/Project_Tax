...
public class DevModeWindow extends JFrame {
    ...
    public DevModeWindow() {
        ...
        userFile = new File("Part_Data/users.csv");
    ...
    private void handleCommand(String cmd) {
        try {
            if (cmd.equalsIgnoreCase("help")) {
                showHelp();
                setStatus("Show all commands", Color.CYAN);
                return;
            }

            if (cmd.equalsIgnoreCase("show users")) {
                showUsers();
                setStatus("Show all user information", Color.CYAN);
                return;
            }
            ...
            if (cmd.toLowerCase().startsWith("show id=")) {
                ...
                showUserById(id);
                setStatus("Show information ID: " + id, Color.CYAN);
                return;
            }
            ...
            if (cmd.toLowerCase().startsWith("add ")) {
                ...
                if (parts.length == 5) {
                    ...
                    log("[+] New user successfully added: " + id);
                    showUsers(); // อัปเดตให้อัตโนมัติ
                    setStatus("New user successfully added: " + id, new Color(0, 255, 100));
                } else {
                    log("รูปแบบผิด: add <id> <fname> <lname> <pass>");
                    setStatus("The add command is not formatted correctly.", Color.RED);
                }
                return;
            }
            ...
            if (cmd.toLowerCase().startsWith("delete ")) {
                ...
                if (parts.length == 2) {
                    deleteUser(parts[1]);
                    showUsers();
                    setStatus("User successfully deleted: " + parts[1], new Color(255, 150, 0));
                } else {
            ...
            if (cmd.toLowerCase().startsWith("edit ")) {
                ...
                if (parts.length == 3 && parts[2].contains("=")) { //ส่วนที่สองต้องมี = ด้วย
                    ...
                    showUsers();
                    setStatus("Successfully edited information: " + id, new Color(0, 200, 255));
                } else {
            ...
            if (cmd.equalsIgnoreCase("clear")) {
                int confirm = JOptionPane.showConfirmDialog(this,
                        "Confirm: Delete all data in users.csv?", "Confirm Clear", JOptionPane.YES_NO_OPTION);
                if (confirm == JOptionPane.YES_OPTION) {
                    clearFile();
                    log("File cleared successfully");
                    setStatus("File cleared successfully", new Color(255, 200, 0));
                } else {
                    log("File clear cancelled");
                    setStatus("File clear cancelled", Color.GRAY);
                }
                showUsers();
                return;
            }
    ...
    private void showHelp() {
        ...
        log("  help");
        log("  show users                - display all user data");
        log("  show id=<id>              - display user data by ID");
        log("  add <id> <fname> <lname> <pass> - add a new user");
        log("  delete <id>               - delete user by ID");
        log("  edit <id> <field>=<value> - edit a field (name|lname|pass)");
        log("  clear                     - clear users.csv (with confirmation)");
        log("  exit                      - close the window");
    }
